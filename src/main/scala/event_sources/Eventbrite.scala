...
//    if (resp != null && resp.body.toString.length > 0) {
  ...
  def getPaginationData(json: JValue): (Int, Int) = {
    ...
    val page_number = (pagination \ "page_number").values.toString
    val page_count = (pagination \ "page_count").values.toString
//    println("String number+count "+page_number + " + "+ page_count)
    val number = page_number.toInt
    val count = page_count.toInt
//    println("Int number + count " +number + " + "+ count)
    (number, count)
  }
...
//      val json = HTTPRequest.getJSON(url, headers=headers)
  ...
  protected def normalizeEvent(eventData: Map[String, Any]): NormalizedEvent = {
    ...
    eventData("ticket_classes") match {
      ...
      case tcs: List[EventbriteTicket] if tcs != null && tcs.length > 0 =>
        val tc = tcs(0)
        if (tc.free)
          fields += (
            "display_price"-> "free",
